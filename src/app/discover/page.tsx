...
import { Search, ChevronRight, Users, Globe, TrendingUp, X } from "lucide-react";
import {
  recommendations,
  users,
  currentUser,
    ...
    >
      ...
      <div className="w-12 h-12 rounded-xl overflow-hidden flex-shrink-0">
        {rec.photo ? (
          ...
          <div className={cn("w-full h-full flex items-center justify-center text-xl", style.bg)}>
  ...
  if (recs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-8 text-center">
        ...
        <p className="text-muted/60 text-xs mt-1">Try a different search or category.</p>
      </div>
    );
  }
...
export default function DiscoverPage() {
  ...
  }, [allRecs, query, activeCategory]);

  // ── sections (unfiltered view) ────────────────────────────────────────────
  ...
  return (
    <>
      <div className="pt-4 pb-4">
        ...
        {/* Category chips */}
        <div className="flex gap-2 overflow-x-auto px-4 pb-1 no-scrollbar mb-5">
          {CATEGORIES.map((cat) => (
            <button
              key={cat}
            ...
            >
              <span className="text-[13px]">{CATEGORY_EMOJI[cat]}</span>
              <span>{cat}</span>
            </button>
          ))}
        </div>

        {/* Content: sections vs flat results */}
        {isFiltered ? (
